...
2. **What `GET /prices` compares against.** §3.2.3 says to "aggregate central database records with store sync logs", so the compared value is `store_sync_status.live_price` (last known). **Revised 2026-09-22 (owner-directed):** a live per-request read is what is wanted, so `GET /prices` now reads both stores (one Admin API query each — every variant as `sku → price`) and compares that against `central_price`; the recorded row is kept for `last_synced_at`. Consequence: a price changed directly in a Shopify admin is flagged on the next page load, at a cost of two Shopify calls per request. T-1.17's script keeps the recorded log level with the stores.
...
- **Do:**
  ...
  3. *(Added 2026-09-22 — assumption 2 revised.)* Export a pure `mergeLivePrices(rows, live)` from `prices.js` and a `readStorePrices(store)` from `shopify.js`, and have the route read both stores before flagging, so the comparison is against what the stores hold now rather than against the last recorded value. A store that cannot be read is reported `failed` on every row and the response stays a `200`.
...
- **Amended 2026-09-22 (owner-directed, assumption 2 revised).** The endpoint no longer compares against the recorded `live_price`. `prices.js` gained the pure `mergeLivePrices(rows, live)` plus one numeric price comparison used by both it and `flagMismatches`; `shopify.js` gained `readStorePrices(store)` (one query returning every variant as `sku → price`); the route reads both stores through `Promise.all` before flagging, so a dashboard load costs two Shopify calls, not twenty. A store that cannot be read is `failed` on every row with its error attached and the response is still a `200`, because the catalogue itself is available. Verified live against the local container: `/prices` → `10` rows and it now flags drift the recorded comparison could not see — Store A holds `99.51` for `SKU-010` against a central `89.51`, so `flagged 1` where the old code reported `synced 89.51`; with `SHOPIFY_BETA_STORE` pointed at a non-existent store the endpoint answered **`200`** with `rows 10 | flagged 10`, `beta statuses: failed | live_price: null`, `error: token request for beta failed: HTTP 404 {"errors":"Not Found"}` and Alpha's own live read intact, then the real domain was restored and `flagged 1` returned. Cost measured: `0.59–1.41s` warm per request (the two Shopify round trips), and `node backend/check-mismatch.js` → `12 cases pass`, exit 0.
...
- **Do:**
  ...
  2. *(Added 2026-09-22.)* Cover `mergeLivePrices` as well, since it now decides what the badges show: a live read that disagrees with a recorded `synced`, one that agrees where the record says otherwise, a read the store refused, a store with no variant for that SKU, and `"22.0"` against `"22.00"`.
...
- **Amended 2026-09-22.** Eight cases added for `mergeLivePrices` (the function that replaced the recorded status with the one just read), so the file now asserts the live-read paths too — recorded `synced` but the store disagrees, recorded drift but the store agrees, a store whose read failed, a store holding no variant for that SKU, and `"22.0"` equal to `"22.00"`. The assertions count themselves, so the printed `check-mismatch: 12 cases pass` (exit 0) cannot drift from the number actually run; the original four cases are unchanged and still first in the file.
- **Blocks:** none
